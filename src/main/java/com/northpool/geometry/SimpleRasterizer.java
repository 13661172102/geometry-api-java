...
public class SimpleRasterizer {
	...
	public long estimateMemorySize()
	{
		...
		long size = SizeOf.SIZE_OF_SIMPLE_RASTERIZER +
				(activeEdgesTable_ != null ? activeEdgesTable_.estimateMemorySize() : 0) +
				(scanBuffer_ != null ? SizeOf.sizeOfIntArray(scanBuffer_.length) : 0);

		if (ySortedEdges_ != null) {
			size += SizeOf.sizeOfObjectArray(ySortedEdges_.length);
		...
		if (sortBuffer_ != null) {
			size += SizeOf.sizeOfObjectArray(sortBuffer_.length);
