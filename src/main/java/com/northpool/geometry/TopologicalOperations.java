...
import com.northpool.geometry.Geometry.GeometryType;
...
final class TopologicalOperations {
	...
	static final class CompareCuts extends AttributeStreamOfInt32.IntComparator {
			...
			if (shape.getGeometryType(geom) == Geometry.Type.Polygon.value()
				...
				if (!dirty_result) {
					((MultiVertexGeometryImpl) polygon._getImpl()).setIsSimple(
							MultiVertexGeometryImpl.GeometryXSimple.Strong, tolerance, false);
					((MultiPathImpl) polygon._getImpl())._updateOGCFlags();
				} else
					((MultiVertexGeometryImpl) polygon._getImpl()).setIsSimple(
							MultiVertexGeometryImpl.GeometryXSimple.Weak, 0.0, false);// dirty result means
																// simple but with 0
																// tolerance.
